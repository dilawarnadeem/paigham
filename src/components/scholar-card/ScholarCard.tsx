  ...
  return (
    <div className='bg-red-300 relative group overflow-hidden'>
      <Link href={`/scholars/${item.slug}?id=${item.databaseId}`}>
        <img src={item?.featuredImage?.node?.mediaItemUrl} alt="image" width={700} height={700} className='w-full object-cover transition-all duration-200 ease-in-out group-hover:scale-105' />
