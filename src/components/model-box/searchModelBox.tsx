    ...
    <div className="bg-black/90 fixed inset-0 z-50">
      <div className="relative w-full h-full">
        <button className="absolute top-6 right-6" onClick={closeModal}>
          <MdOutlineClose size={35} color="white" />
        </button>
          ...
          <div className="bg-white md:flex item-center md:rounded-full p-1">
            <div className="flex item-center w-full">
              <input
                ...
                className="bg-transparent font-metapro text-lg p-3 px-6 outline-none w-full"
              />
              <button
                onClick={handleSearch}
                className="bg-secondary text-2xl px-3.5 hover:bg-black hover:text-secondary rounded-full"
              >
                <FiSearch />
              </button>
            </div>
...
const SelectFilter = ({ setCategory, setOpenCategory }: any) => {
  return (
    <div className="rounded-2xl overflow-hidden outline-none p-4 w-full md:w-[160px] absolute top-16 left-0 right-0 bg-white shadow-lg">
      <button
        className="block py-1 border-b-[1px] border-gray-100 w-full text-left hover:text-secondary cursor-pointer"
        onClick={() => {
          setCategory("Ramadan");
