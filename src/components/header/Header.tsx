  ...
  return (
    <>
      ...
      <header className="sticky top-0 z-[50]">
        {/* main nav   */}
        <nav
          className={`flex justify-between items-center py-4 px-6 z-50 bg-gradient-to-l  from-blue via-blue to-[#0F275E] ${
            isScrolled && "bg-gradient-to-l  from-blue via-blue to-[#0F275E]"
          }`}
        >
          ...
          <div className="text-white flex items-center gap-4 z-10">
            <button
              className="lg:hidden w-[33px] flex justify-center items-center h-[33px]"
          ...
          <div
            className={`lg:flex items-center lg:space-x-3 ${
              isMobileNav
                ? "block absolute top-[0px] p-6 pt-20 md:pt-40 pb-10 left-0 right-0 z-[1] bg-primary"
                : "hidden"
            }`}
          >
            <ul className="lg:flex items-center text-white gap-8">
              {navList.map((item, idx) => (
                <li
                  key={idx}
                  className="font-metapro flex items-center gap-2 hover:text-secondary cursor-pointer py-3 md:py-0 tracking-wide font-semibold capitalize text-lg text-pure"
                  onClick={() => NavController(item.link)}
                >
                  {item.name}
                  {item?.icon && (
                    <img
              ...
              <li
                className={`hidden cursor-pointer md:block pr-4 rtl:pr-0 ${
                  documentDirection === "rtl" && "pl-4"
                }`}
              >
                <i className="">
                  <FiSearch size={24} onClick={() => OpenSearch()} />
      ...
      {SearchModalIsOpen && (
        <SearchModelBox setSearchModalIsOpen={setSearchModalIsOpen} />
      )}
